...
int read_value(Arena *arena, uint32_t value_type, gguf_metadata_value_t *value, FILE *f)
{
	...
	case GGUF_METADATA_VALUE_TYPE_ARRAY:{
	    ...
	    for (uint64_t i = 0; i < len; i++) {
    		if (!read_value(arena, type, &value->array.array[i], f)) {
		    return 0;
    		}
	    }
	    break;
...
int read_kv(Arena *arena, gguf_metadata_kv_t *metadata_kv, uint64_t kv_count, FILE *f)
{
	...
	err = read_value(arena, value_type, &metadata_kv[i].value, f);
	if (!err) return 0;
    }
...
gguf_header_t *read_header(Arena *arena, const char *file_name){
    ...
    if (fread(&tensor_count, sizeof(tensor_count), 1, f) != 1) return NULL;

    uint64_t kv_count;
    if (fread(&kv_count, sizeof(kv_count), 1, f) != 1) return NULL;
