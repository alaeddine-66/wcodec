...
int strmap_init(StrMap *m, Arena *arena, size_t initial_cap) {
    ...
    while (cap < min) cap <<= 1;
    m->entries = (StrMapEntry *)arena_alloc(arena, cap * sizeof(StrMapEntry));
    if (!m->entries) return 0;
    memset(m->entries, 0, cap * sizeof(StrMapEntry));
    m->cap = cap;
    m->len = 0;
    return 1;
}

int strmap_put(StrMap *m, const char *key_owned, int value) {
